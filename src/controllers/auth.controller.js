...
export async function signIn(req, res) {
    ...
    try {
        ...
        if (emailExists.rows.length === 0) return res.status(401).send("Usuário/senha incorretos!")
        const isCorrectPassword = bcrypt.compareSync(password, emailExists.rows[0].password)
        if (!isCorrectPassword) return res.status(401).send("Usuário/senha incorretos!")
